class Loader(BasicDataset):
    ...
    def get_adj(self):
        ...
        graph = graph.tocsr()
        graph = self._convert_sp_mat_to_sp_tensor(graph)
        adj = graph.coalesce().to(world.device)
        return adj
    ...
    def pre_epoch_processing(self):
        ...
        if self.pruning_random:
            # pruning randomly
            keep_idx = torch.tensor(random.sample(range(self.edge_values.size(0)), keep_len)).to(world.device)
        else:
        ...
        self.masked_adj = torch.sparse.FloatTensor(all_indices, all_values, self.norm_adj.shape).to(world.device)
        sp.save_npz(self.path + '/s_pre_adj_mat.npz', self.masked_adj)
